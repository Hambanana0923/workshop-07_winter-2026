...
# creating a new object called abalone_clean
...
ggplot(data = abalone_clean, # using abalone_clean data frame
       aes(x = mean_ph, # x-axis = predictor, y-axis = response
           y = change_in_area)) +
  # first layer: points to represent average cgange in shell surface area for each pari of tanks
  geom_point(size = 4, 
             stroke = 1,
             fill = "firebrick4",
             shape = 21)
...
par(mfrow = c(2, 2))   # display resisual plots in 2x2 grid
plot(abalone_model)    # plot the residuals

```

**Personal Notes:**Residuals seem homoscedastic (consistent across values) based on Residuals vs Fitted and Scale_location plots. Residuals also seem normally distributed based on QQ plot. No major outliers based on Residuals vs. Leverage plot.
...
**-18.5 +- 6.16 (SE). Whem pH = 0, the model predicts that abalone change in shell surface area per day is -18.5 +- 6.16 (Note to self: this is not necessarily biologically realistic, but is something the model generates as an estimate.) **
...
**std.error: standard error, precision of the preiction**

**conf.low and conf.high: range for the 95% CU ariybd model prediction**
...
**This object is ggeffects output object and a data frame**  
...
**At pH = 8, the model predicts that abalone hange in shell surface area per day should be 5.36 mm^2^ d^-1^ (95% CI: [5.08, 5.64]).**
